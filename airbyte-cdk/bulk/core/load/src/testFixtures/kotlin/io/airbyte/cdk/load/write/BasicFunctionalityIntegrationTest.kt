...
abstract class BasicFunctionalityIntegrationTest(
    ...
    open fun testMidSyncCheckpointingStreamState() =
        runBlocking(Dispatchers.IO) {
            ...
            stateMessages.forEach { stateMessage ->
                ...
                if (verifyDataWriting) {
                    val records = dataDumper.dumpRecords(streamName, streamNamespace)
                    val expectedId =
                        when (streamName) {
                            "test_stream1" -> 12
                            "test_stream2" -> 34
        ...
        val catalog =
            DestinationCatalog(
                listOf(
                    ...
                    makeStream(
                        "groups",
                        linkedMapOf("id" to intType, "authorization" to intType)
                    ),
                )
            )
        // For each stream, generate a record containing every field in the schema
        val messages = catalog.streams.map {
            DestinationRecord(
                it.descriptor,
                ObjectValue(
                    ...
                    dumpAndDiffRecords(
                        listOf(
                            OutputRecord(
                                extractedAt = 1234,
                                generationId = 0,
                                data = (stream.schema as ObjectType).properties.mapValuesTo(linkedMapOf()) { 42 },
    ...
    open fun testTruncateRefresh() {
        ...
        runSync(
            config,
            makeStream(generationId = 13, minimumGenerationId = 13, syncId = 43),
            listOf(
                DestinationRecord(
                    randomizedNamespace,
                    "test_stream",
