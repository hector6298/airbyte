...
class DestinationTaskScopeProvider(config: DestinationConfiguration) :
    ...
    override suspend fun launch(task: WrappedTask<ScopedTask>) {
        when (task.innerTask) {
            is InternalScope -> internalScope.scope.launch { execute(task, internalScope) }
            is ImplementorScope -> implementorScope.scope.launch { execute(task, implementorScope) }
        }
    }

    private suspend fun execute(task: WrappedTask<ScopedTask>, scope: ControlScope) {
